-- ============================================================
-- 016. Atomic composite mutations
-- Run AFTER 015_admin_read_allowlist.sql.
--
-- These functions eliminate partial-write windows for composite resources:
-- tasks + subtasks and goals + milestones.
-- The caller must be the same user as p_user_id, unless the request is made
-- with the trusted service_role used by the MCP/API-key server path.
-- ============================================================

CREATE OR REPLACE FUNCTION public.is_trusted_user_context(p_user_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
STABLE
AS $$
...
BEGIN
  ...
  VALUES (
    COALESCE(NULLIF(p_task->>'id',''), gen_random_uuid()::text),
    p_user_id,
    p_task->>'title',
    p_task->>'description',
...
BEGIN
  ...
  SET
    ...
    project_id = CASE WHEN p_task ? 'project_id' THEN NULLIF(p_task->>'project_id','') ELSE project_id END,
    ...
    depends_on = CASE WHEN p_task ? 'depends_on' THEN p_task->>'depends_on' ELSE depends_on END,
...
BEGIN
  ...
  VALUES (
    COALESCE(NULLIF(p_goal->>'id',''), gen_random_uuid()::text),
    p_user_id,
