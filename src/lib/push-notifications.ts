...
// ============================================================
// push-notifications.ts — Web Push من جهة العميل
//
// اشتراك كل جهاز/متصفح على حدة في إشعارات Web Push (upsert حسب
// endpoint) بمفتاح VAPID عام يجلب من الخادم لا من env. تستهلكه
// شاشة الإعدادات وقسم الإشعارات فيها، وpwa-init يستدعي منه إعادة
// مزامنة الاشتراك القائم فقط.
//
// المسؤوليات:
//   1) getPushStatus: قراءة الحالة الكاملة (دعم المتصفح/الصلاحية/
//      اشتراك قائم/تهيئة الخادم) دون طلب أي صلاحية أو تعديل.
//   2) enablePush: من إيماءة مستخدم فقط — صلاحية ← مفتاح الخادم ←
//      Service Worker ← subscribe ← تسجيل على الخادم، مع فك الاشتراك
//      المحلي إذا رفض الخادم حتى لا نظن أننا مفعّلون.
//   3) disablePush (يبطل القناة لا مركز الإشعارات الداخلي) و
//      resyncPushSubscription وdeviceLabel (تسمية ودّية للأجهزة).
//
// حدود: لا يطلب الصلاحية تلقائياً عند التحميل أبداً ويشترط HTTPS
// (عدا localhost)؛ خلفية التصميم في التعليق أعلى الملف.
// ============================================================
