...
/* ────────────── Horizon Dial (signature element) ──────────────
   A dawn-to-dusk arc: the fill traces today's completion %, and a
   marker travels along it to mark the actual time of day. Literally
   "own your morning" made visible instead of a generic donut chart. */

function HorizonDial({ completion }: { completion: number }) {
  const size = 132
  const strokeWidth = 9
  ...
  const offset = circumference - (Math.min(100, Math.max(0, completion)) / 100) * circumference
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size / 2 + 8 }}>
      <svg width={size} height={size / 2 + 8} viewBox={`0 0 ${size} ${size / 2 + 8}`}>
        ...
        <path d={pathD} fill="none" className="stroke-primary/10" strokeWidth={strokeWidth} strokeLinecap="round" />
        <motion.path
          d={pathD}
          fill="none"
        ...
        <motion.circle
          ...
          cy={markerY}
          r={5}
          className={isDay ? 'fill-gold' : 'fill-forest'}
          initial={{ opacity: 0, scale: 0 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.9, duration: 0.4, type: 'spring' }}
          style={{ filter: isDay ? 'drop-shadow(0 0 6px var(--gold))' : 'drop-shadow(0 0 6px var(--forest))' }}
        />
      </svg>
      <div className="absolute inset-x-0 bottom-0 flex flex-col items-center">
        <span className="text-lg font-bold text-foreground leading-none">{toArabicNum(Math.round(completion))}٪</span>
        <span className="text-[9px] text-muted-foreground mt-0.5">تقدم اليوم</span>
      </div>
    </div>
  )
}
      ...
      <motion.div variants={itemVariants} className="relative">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <h1 className="text-2xl lg:text-3xl font-bold tracking-tight text-gradient-forest font-display">{displayName}</h1>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            {/* Horizon Dial — today's completion traced along a dawn-to-dusk arc */}
            <TooltipProvider>
              <Tooltip>
