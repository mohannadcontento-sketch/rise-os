...
// ============================================================
// reminder-engine.tsx — محرك التذكيرات العام
//
// مكوّن أثري خالص (يُرجع null) يُحمَّل lazy ويُركَّب مرة واحدة
// في هيكل التطبيق (app/page.tsx) فتعمل التذكيرات على أي وحدة:
// تذكير الاستيقاظ والنوم (من rise-settings المحلية) وتذكيرات
// العادات (من /api/rise/habits عبر apiGet). كل تذكير يُطلق مرة
// واحدة في اليوم كحد أقصى (حارس getUserStorage
// 'rise-reminders-fired' يتجدد مع تغير التاريخ).
//
// البنية الداخلية:
//   1) أدوات اليوم/الوقت + تحميل/حفظ حالة «المُطلقة»
//   2) ReminderEngine — جلب العادات كل 5 دقائق (يتخطى التبويب
//      المخفي ويجلب فور عودته) + إعادة جلب موجّهة عند
//      rise:data-changed فقط عندما resource=habits
//   3) fireReminder — toast 7 ثوان بزر «افتح» (يبثّ
//      rise:navigate) + إشعار متصفح force + pushNotification
//      لصندوق جرس الإشعارات
//   4) check — نبضة كل 15 ثانية تقارن HH:MM الحالي بأوقات
//      الاستيقاظ/النوم/العادات مع احترام تفضيلات
//      isNotificationEnabled (morning/sleep/habits)
//
// مبادئ تقنية: إشعار المتصفح يظهر هنا حتى مع التركيز (التذكير
// هو الغاية، خلافًا لجرس الإشعارات)؛ النبضات الزائدة غير
// ضارة بفضل حارس المرة الواحدة؛ قفل checkingRef يمنع تداخل
// دورات الفحص؛ إعادة تعيين الحارس عند تغير اليوم داخل check.
// ============================================================
