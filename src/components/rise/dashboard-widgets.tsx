...
// ============================================================
// dashboard-widgets.tsx — ودجات لوحة القيادة الغنية
//
// القسم الغني من الصفحة الرئيسية (home.tsx): يستعيد كل ودجات
// لوحة التحكم المحبوبة — درجة الإنتاجية الموحّدة، KPIs اليوم
// (صباح/مهام/عادات/تركيز)، المهام المتأخرة والقادمة، العادات،
// الأهداف، الصحة، الشارات، وجدار التحفيز — والجلب كلّه عبر
// use-dashboard-data (نداء تجميعي واحد /api/rise/dashboard).
//
// يُحمَّل lazy بعد خمول المتصفح (requestIdleCallback) ضمن حزمة
// منفصلة مع framer-motion وrecharts — فلا يمس LCP فوق الطية.
// الترحيب والمستوى/الخبرة في هيدر home.tsx (الموجة الأولى).
//
// تحسين على الأصل: تسجيل العادة من اللوحة مباشرة (نقرة واحدة).
//
// البنية الداخلية:
//   1) أدوات مساعدة: أرقام عربية، تحويل أيام/أولويات/مزاج،
//      واقتباسات التحفيز
//   2) مكونات رسم صغيرة قابلة لإعادة الاستخدام: MiniSparkline،
//      AnimatedNumber، CircularProgress، Skeleton، ChartTooltip،
//      SectionHeader، PremiumGlass
...
//   5) DashboardWidgets: جلب → تطبيع → تخطيط متدرّج (درجة،
...
import { apiFetch, apiPut } from '@/lib/api-fetch'
...
function ProductivityScoreCard({ fallbackScore, breakdown: serverBreakdown, todayData, streak = 0 }: { fallbackScore?: number; breakdown?: { tasks: number; habits: number; focus: number; morning: number; streak: number }; streak?: number; todayData?: { tasksCompleted: number; tasksTotal: number; habitsCompleted: number; habitsTotal: number; focusMin: number; morningScore: number } }) {
  ...
  // Gradient color based on score — uses the Horizon palette (Pine → good, Gold → okay, red → needs work)
  const gradientId = 'prodScoreGradient'
  let strokeColor1 = 'var(--forest)'
  let strokeColor2 = 'var(--forest-dark)'
  let glowColor = 'rgba(58, 90, 80, 0.25)'
  let gradeColor = 'text-forest'

  if (score >= 70) {
    strokeColor1 = 'var(--forest)'
    strokeColor2 = 'var(--forest-dark)'
    glowColor = 'rgba(58, 90, 80, 0.25)'
    gradeColor = 'text-forest'
  } else if (score >= 50) {
    strokeColor1 = 'var(--gold)'
    strokeColor2 = 'var(--gold-light)'
...
export default function DashboardWidgets() {
  ...
  const { data, loading, error, fromCache, fetchDashboard, movingId, moveOverdueToToday, todayDate } = useDashboardData()
  ...
  // ── القسم: تسجيل العادة من اللوحة مباشرة (نقرة واحدة §10) ──
  ...
  const toggleHabit = useCallback(async (habitId: string, currentlyDone: boolean) => {
    ...
    try {
      // apiPut يبث rise:data-changed → الجلب يتحدث تلقائيًا
      await apiPut('/api/rise/habits', { habitId, date: todayDate, completed: !currentlyDone })
    } catch {
  ...
  // ── القسم: مشتقات العرض — الشارات والقوائم والاتجاهات ──
                  ...
                  <AreaChart data={chartData} margin={{ top: 10, right: 0, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="scoreGradient" x1="0" y1="0" x2="0" y2="1">
                        ...
                        <stop offset="40%" stopColor="var(--forest)" stopOpacity={0.15} />
                    ...
                    <Area
                      ...
                      dot={{ r: 4, fill: 'var(--forest)', strokeWidth: 2, stroke: 'var(--card)' }}
                      activeDot={{ r: 6, fill: 'var(--forest)', strokeWidth: 2, stroke: 'var(--card)' }}
                      name="الدرجة"
                    />
                  </AreaChart>
                </ResponsiveContainer>
                  ...
                  {habits.map((habit, i) => (
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={() => toggleHabit(habit.id, habit.todayCompleted)}
                        disabled={habitBusyId === habit.id}
                        aria-label={habit.todayCompleted ? `إلغاء تسجيل العادة: ${habit.name}` : `تسجيل العادة: ${habit.name}`}
                        className={cn(
