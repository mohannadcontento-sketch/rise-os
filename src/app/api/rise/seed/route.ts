...
// ============================================================
// /api/rise/seed — أداة تطوير (بيانات تجريبية)
//
// يملأ حساب المستخدم ببيانات نموذجية غنية لتجربة الوحدات
// كاملة: مشاريع ومهام، عادات مع سجلات 30 يوماً، أهداف
// ومحطات، يوميات، جلسات تركيز، صحة، مالية، كتب، معرفة،
// درجات يومية وإنجازات — للتطوير والاختبار فقط.
//
// الحماية: 404 قسري في production (NODE_ENV)؛ ولا يُستخدم
//        service_role أبداً — الزرع بحساب المستخدم نفسه عبر
//        طبقة RLS كي لا يُرفَع دور مستخدم عادي.
// الطرق: POST { createProfileOnly? } — زرع كامل إن كان الحساب
//        فارغاً أو إنشاء الملف فقط؛ يرد { seeded } بمصير
//        العملية، والفشل 503 (لا نجاح زائف — إعادة المحاولة
//        آمنة).
// Idempotency-Key: مطلوب (withIdempotency).
// ملاحظة: الصفوف الكثيفة (habit_logs, milestones,
//        daily_scores) تُدرَج مباشرة دفعات من 100 عبر عميل
//        المستخدم المُصادَق.
// ============================================================
