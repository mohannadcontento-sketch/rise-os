...
1. ~~**تطبيق migration 026** في Supabase SQL Editor~~ — **تم بالنيابة عنك (Super Z، 2026-09-12)**:
   اتصال مباشر بقاعدة البيانات بصلاحيات postgres (وفّرها المالك) —
   تحقق كامل من كل الـmigrations 001→026: كلها متطبقة ✓ (الجداول، الـRPCs، الأعمدة، الفهارس).
2. ~~**(اختياري) تشغيل اختبار SQL**~~ — **تم بالنيابة عنك (2026-09-12)**:
   `phase4_usage_limits.sql` → **كل المجموعات الثماني ناجحة** (11 سطر نجاح) و
   `phase5_notifications.sql` → **كل المجموعات الأحد عشر ناجحة** (17 سطر نجاح).
   كلاهما داخل BEGIN…ROLLBACK — تحقق ما بعد التشغيل: صفر أثر على بيانات الإنتاج.
   أثناء التشغيل الفعلي اكتُشفت وأُصلحت 3 أخطاء كامنة في ملفات الاختبار نفسها
   (إعادة SET ROLE قبل تعبئة العدادات، و% حرفية داخل RAISE) + خطأ إنتاج حقيقي (انظر migration 027).
...
> **Hotfix 027 (2026-09-12)**: تشغيل الاختبارات على قاعدة البيانات الحقيقية كشف أن
> `get_notifications_feed` كانت معرّفة `STABLE` بينما تنفّذ DELETE كسولًا → كل نداء لها
> كان يفشل والتطبيق يتراجع صامتًا للمسار القديم (مركز الإشعارات كان degraded فعليًا).
> الإصلاح: `supabase/migrations/027_fix_get_notifications_feed_volatile.sql`
> (VOLATILE) — **متطبق في الإنتاج ومتحقق منه**؛ بعده اجتازت الاختبارات الإحدى عشرة كاملة.
