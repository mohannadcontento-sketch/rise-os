...
> **آخر تحديث:** 2026-09-14 (جلسة 3) · تحقق حي من خطوة المالك للـ Upstash: تشخيص قاطع من 3 مستويات (بعد الـ Redeploy وبعد بناء جديد كليًا 28e7634) = المتغيران غير مضبوطين في Vercel dashboard — الكود والمكتبات سليمان 100% · التفاصيل في `docs/phase-14/UPSTASH_RETEST.md`
...
> 🔴 **Upstash (الخطة المجانية): تشخيص نهائي قاطع 2026-09-14 (جلسة 3):** الكود سليم (429 داخل نفس العزل) + المكتبات مثبتة + **بناء جديد كليًا من commit (28e7634) لا يزال بلا 429 موزّع** ← المتغيران غير مضبوطين في Vercel dashboard نفسه (المشروع/البيئة/الأسماء). الإصلاح الفصل في أوامر المالك رقم 3 + الدليل الكامل في `docs/phase-14/UPSTASH_RETEST.md`.
...
⚠️ الحدود: كود الحدود مُتحقق منه (429 داخل نفس العزل، بالرسالة والرأس الصحيحين) · 🔴 متغيرا Upstash غير موجودين في Vercel dashboard (تم استبعاد الكود والـ Redeploy نهائيًا ببناء جديد كليًا 28e7634) — خطوة المالك رقم 3 معدّلة بالخطوات الفاصلة.
...
3. **🔴 Upstash — التشخيص النهائي (جلسة 3): استُبعد الكود والـ Redeploy نهائيًا** (بناء جديد كليًا من commit 28e7634 لا يزال بلا عدّ موزّع ← المتغيران غير موجودين في الـ dashboard). المطلوب بالحرف:
   1. Vercel → **المشروع الذي يخدم `rise-os-gamma.vercel.app` تحديدًا** (لو عندك أكثر من مشروع — ده السبب الأشهر للفشل، تأكد من الدومين الفرعي داخل صفحة المشروع قبل أي خطوة).
   2. Settings → Environment Variables → Add:
      - `UPSTASH_REDIS_REST_URL` = من Upstash Console → قاعدة بياناتك → قسم **REST API** → `https://…​.upstash.io` (وليست `redis://…`).
      - `UPSTASH_REDIS_REST_TOKEN` = التوكن من نفس القسم.
      - Environment: **Production** (ويفضّل أيضًا Preview + Development).
   3. قل «تم» فقط — سيُدفع marker-commit لإجبار بناء جديد يقرأ المتغيرين (أضمن من Redeploy)، ثم يُنفَّذ الفحص الحي فورًا: 6 محاولات دخول خاطئة بطلبات منفصلة → السادسة 429 = مغلق ✅.
