...
## المرحلة 15 — Beta مغلقة 🚀 انطلقت (جلسة 4)
**قناة الملاحظات داخل التطبيق (البند الناقص الوحيد) — مبنية ومنشورة ومتحقق منها حيًا:**
- هجرة **037** (جدول feedback + RLS: إدراج/قراءة باسمك فقط — لا تعديل/حذف من جهة العميل) 👤 بانتظار تطبيق المالك من SQL Editor
...
👤 المتبقي على المالك: تطبيق 037 → التأكد من إيقاف وضع الصيانة إن كان اختبارًا → استدعاء أول 5-20 مستخدمًا.
...
5. **Beta (انطلقت):** (أ) طبّق `supabase/migrations/037_phase15_feedback.sql` من SQL Editor — (ب) لو وضع الصيانة المفتوح حاليًا اختبار، أوقفه من تاب «النظام» — (ج) استدعِ أول 5-20 مستخدمًا حقيقيًا + اتبع قائمة الفحص اليومي في `docs/phase-15/BETA_RUNBOOK.md` → ثم الدومين awj.life أخيرًا.
