...
> 🔴 **Upstash (الخطة المجانية) ربطه المالك 2026-09-14 — لكن المتغيرين غير فعّالين في Production (تشخيص قاطع: 429 يظهر داخل نفس العزل = الكود سليم؛ ولا يظهر عبر اتصالات منفصلة = العدّ الموزع غائب).** الإصلاح في أوامر المالك رقم 3.
...
⚠️ الحدود: كود الحدود مُتحقق منه (429 داخل نفس العزل) · 🔴 متغيرا Upstash غير فعّالين في Production — خطوة المالك رقم 3 (إضافة المتغيرين + Redeploy).
...
3. **🔴 Upstash (تم الربط لكن غير فعّال):** التشخيص أثبت أن الكود يعمل (429 داخل نفس العزل) والمتغيرين لا يصلان لـruntime الإنتاج. المطلوب: Vercel → مشروع rise-os → Settings → Environment Variables → إضافة `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` (قيم **REST API** من Upstash — `https://…upstash.io` — وليس `redis://`) لبيئة **Production** → ثم Deployments → أحدث نشر → **Redeploy**. التحقق: 6 محاولات دخول خاطئة بطلبات منفصلة → السادسة 429 (قل لي وأنفذ الفحص).
