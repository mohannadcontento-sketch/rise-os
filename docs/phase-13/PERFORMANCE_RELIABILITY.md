...
> **الحالة: مكتملة** (بالأدلة أدناه) — تاريخ الإغلاق: 2026-09-14 · تحديث Upstash: 2026-09-14 (جلسة 2)
...
- 🔴 **Upstash (الخطة المجانية) ربطه المالك 2026-09-14 — لكن المتغيرين غير فعّالين في Production بعد deploy جديد.** التشخيص القاطع: burst باتصال واحد يعطي 429 في المحاولة 6 (الكود يعمل 100% داخل العزل الواحد) بينما اتصالات منفصلة لا تعطيه (كل اتصال = عزل جديد = عدّاد يصفّر). **الإصلاح:** Vercel → Settings → Environment Variables → إضافة `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN` (قيم REST API من Upstash وليس redis://) لبيئة **Production** → ثم **Redeploy**. التحقق: 6 محاولات دخول خاطئة بطلبات منفصلة → السادسة = 429.
